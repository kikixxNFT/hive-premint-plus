...
export async function fetchStatus({
  ...
  url: string;
  wallet: string;
}): Promise<{ url: string; status: RaffleData['status'] }> {
  const { data } = await axios.get(`${url}/verify/?wallet=${wallet}`);
  const parser = new DOMParser();
  const document = parser.parseFromString(data, 'text/html');
  const textStatus = document?.querySelector(
    '.card .card-body div:nth-child(2)'
  )?.textContent;
  const emojiStatus = document?.querySelector(
    '.card .card-body div:nth-child(1)'
  )?.textContent;
  const status = textStatus?.includes(UNREGISTERED.wording)
    ? UNREGISTERED.status
    : statuses[emojiStatus || 'unknown'];
  return { url, status };
}

export function useGetPremintStatus({
  url,
  wallet,
}: {
  url: string;
